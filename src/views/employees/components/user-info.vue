<template>
  <div class="user-info">
    <i
      @click="
        $router.push(`/employees/print/${userId}?type=personal`)
      "
...
import uploadImg from '../components/myUpLoadImg'
...
export default {
  ...
  methods: {
    async loadUserDetail() {
      this.userInfo = await userInfo(this.userId)
      this.$refs.headerPic.fileList.push({
    ...
    async onSaveUserDetail() {
      if (this.$refs.headerPic.loading) {
        return this.$message.error('上次图片中，请稍后')
      }
      await userInfo(this.userId)
