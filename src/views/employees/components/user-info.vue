      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <UploadImg @onSuccess='onSuccess' ref="headerImage"></UploadImg>
            <!-- 放置上传图片 -->
          </el-form-item>
        </el-col>
      </el-row>
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <UploadImg ref="employeesImage" @onSuccess='onEmployeesImageSuccess'></UploadImg>
        </el-form-item>
      ...
      <div class="block">
        ...
        <el-row class="inline-info" type="flex" justify="center">
          <el-col :span="12">
            <el-button type="primary" @click="SaveUpdatePersonal">保存更新</el-button>
            <el-button @click="$router.back()">返回</el-button>
          </el-col>
        </el-row>
      </div>
...
import { saveUserDetailInfoApi, getUserDetailInfoApi } from '@/api/user'
import { getPersonalDetailApi, updatePersonalApi } from '@/api/employees'
...
export default {
  ...
  methods: {
    async getUserDetailInfo() {
      this.userInfo = await getUserDetailInfoApi(this.userId)
      this.$refs.headerImage.imageList.push({
        url: this.userInfo.staffPhoto,
      })
    },
    async getPersonalDetail() {
      this.formData = await getPersonalDetailApi(this.userId)
      this.$refs.employeesImage.imageList.push({
        url: this.userInfo.staffPhoto,
      })
    },
    async onSAveUserDetail() {
      if (this.$refs.headerImage.loading) {
        this.$message.error('图片正在上传中')
      }
      await saveUserDetailInfoApi(this.userInfo)
      this.$message.success('更新成功')
    },
    async SaveUpdatePersonal() {
      if (this.$refs.employeesImage.loading) {
        this.$message.error('图片正在上传中')
      }
    ...
    onSuccess({ url }) {
      console.log('发送请求A', url)
      this.userInfo.staffPhoto = url
    },
    onEmployeesImageSuccess({ url }) {
